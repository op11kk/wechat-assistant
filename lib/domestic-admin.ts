...
export async function getDomesticManageData(): Promise<DomesticManageData> {
  const [users, leaders, collectors] = await Promise.all([
    dbQuery<{
      ...
      `select id, phone, role, status, display_name, real_name, created_at, last_login_at
       from ${webUsersTable}
       order by id desc
       limit 300`,
    ),
...
export async function updateDomesticUser(params: {
  ...
  const row = await dbQueryMaybeOne<{ id: unknown }>(
    `update ${webUsersTable}
     ...
     where id = $5
     returning id`,
    [params.role, params.status, params.displayName ?? "", params.realName ?? "", params.userId],
  );
      ...
      const user = (
        await client.query<{ id: unknown }>(
          `insert into ${webUsersTable} (
             ...
             real_name,
             legal_agreed_at
           ) values ($1, 'leader', 'active', $2, $2, now())
           ...
           set role = 'leader',
               ...
               updated_at = now()
           returning id`,
          [phoneResult.phone, promoterName],
        )
      ).rows[0];
      ...
      const user = (
        await client.query<{ id: unknown }>(
          `insert into ${webUsersTable} (
             ...
             invited_by_user_id,
             legal_agreed_at
           ) values ($1, 'collector', 'active', $2, $2, $3, now())
           on conflict (phone) do update
           set role = 'collector',
               status = 'active',
               display_name = excluded.display_name,
               ...
               updated_at = now()
           returning id`,
          [phoneResult.phone, realName, params.actorUserId ?? null],
        )
      ).rows[0];
        ...
        await client.query(
          `update ${webUsersTable}
           set display_name = coalesce(nullif($1::text, ''), display_name),
               ...
               updated_at = now()
           where id = $4`,
          [realName ?? "", phoneResult?.ok ? phoneResult.phone : null, params.status ?? null, userId],
        );
      }
