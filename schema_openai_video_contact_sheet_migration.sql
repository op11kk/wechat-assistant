-- Incremental migration for the COS-local contact-sheet preprocessing pipeline.
-- Safe to run repeatedly. Apply after the base video collector tables exist.
...
CREATE TABLE IF NOT EXISTS public.video_submission_artifacts (
  ...
  CONSTRAINT ux_video_submission_artifacts_version_segment UNIQUE (
    submission_id,
    preprocess_version,
    artifact_type,
    segment_index
...
COMMENT ON TABLE public.video_submission_artifacts IS
  'Deterministic preprocess outputs such as contact sheets used by OpenAI Batch review.';
COMMENT ON TABLE public.openai_video_review_batches IS
  'OpenAI Batch jobs for asynchronous video review.';
COMMENT ON TABLE public.openai_video_review_batch_items IS
  'Submission-level OpenAI Batch requests and results.';
