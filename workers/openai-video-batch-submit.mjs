...
function buildReviewPrompt(submission, sheetCount) {
  const durationSec = Number(submission.duration_sec ?? 0);
  const durationMin = durationSec > 0 ? (durationSec / 60).toFixed(1) : "";
  return [
    "你正在审核一段用于机器人训练的第一视角视频。",
    `submission_id: ${submission.id}`,
    `participant_code: ${submission.participant_code || ""}`,
    `file_name: ${submission.file_name || ""}`,
    `视频时长（秒）: ${submission.duration_sec || ""}`,
    `视频时长（分钟，保留1位小数）: ${durationMin}`,
    `联系图数量: ${sheetCount}`,
    "图片按时间顺序排列，每个小图都带有时间戳。",
    "请判断：1）是否为第一视角；2）双手同时清晰可见并可用于学习的比例；3）清晰度和视野是否足够用于训练双手操作。",
    "decision 只能是 auto_pass、auto_reject、review_needed 之一。",
    "ratio 返回 0 到 1 之间的小数，表示双手同时可见且可用于学习的比例。",
    "summary 必须是纯中文、1到2句、简洁、不超过50个汉字，并包含视频时长（按分钟表达）。",
    "final_conclusion_cn 也必须是纯中文、简洁，表达和 summary 一致即可。",
    "只返回严格 JSON，不要输出任何额外文字。JSON 键固定为：decision, ratio, confidence, summary, basic_info, hand_visibility, issue_timeline, hard_fail_reasons, final_conclusion_cn。",
  ].join("\n");
}
